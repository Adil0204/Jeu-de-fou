...
posvoiture = [275, 650]
lanes_x = [175, 275, 375]  # Positions centrées pour chaque voie
lane_index = 1             # On commence au milieu
posvoiture[0] = lanes_x[lane_index]
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if abs(posvoiture[0] - cible_x) < seuil_alignement:# sert à empêcher que le joueur puisse changer de voie tant qu’il n’est pas bien aligné avec une voie avec une marge d'erreur sinon c'est nul
                if event.key == pygame.K_LEFT and lane_index > 0:#on chèque bien que c'est possible d'aller vers la voie cible (faut pas sortir de la route sinon vous allez écraser quelques vaches et deux trois enfants et c'est pas génial)
                    lane_index -= 1
                    cible_x = lanes_x[lane_index]
